...
const elderlyPlacesService = {
  getElderlyPlacesData: async (lat, lng) => {
    try {
      ...
      console.log(`노약자 장소 요청 위치: lat=${lat}, lng=${lng}`);
      ...
      console.error('복지시설 데이터 가져오기 실패:', error);
      throw new Error('복지시설 데이터 가져오기 실패: ' + error.message);
    }
  }
};

module.exports = elderlyPlacesService;
