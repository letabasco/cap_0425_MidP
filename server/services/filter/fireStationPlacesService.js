...
const fireStationPlacesService = {
  getFireStationPlacesData: async (lat, lng) => { // renamed function
    try {
      ...
      console.log(`경찰서 장소 요청 위치: lat=${lat}, lng=${lng}`);
      
      // 카카오 로컬 키워드 검색 API 호출
      const url = 'https://dapi.kakao.com/v2/local/search/keyword.json';
      const params = new URLSearchParams({
        query: '119소방서서', // 검색 키워드
        x: lng,          // 경도
        y: lat,          // 위도
        radius: 5000,    // 5km 반경
        size: 15         // 검색 결과 수
      ...
      console.error('소방서 데이터 가져오기 실패:', error);
      throw new Error('소방서 데이터 가져오기 실패: ' + error.message);
    }
  }
};

module.exports = fireStationPlacesService;
